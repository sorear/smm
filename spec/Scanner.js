...
function segs(ss) {
    it(`has ${ss.length} statements`, function () { expect(db.statements.length).toBe(ss.length); });
    ss.forEach(function (s,ix) {
        it(`statement ${ix}=${s.typ}`, function () { expect(seg2(db,ix)).toEqual([MMOM.Statement[s.typ],s.raw,s.mat,s.prf,s.lbl]); });
        if (s.stp) { it(`statement ${ix}/startPos`, function () { expect(pos(db,ix,'startPos')).toEqual(s.stp); }); }
        if (s.map) { it(`statement ${ix}/mathPos`, function () { expect(pos(db,ix,'mathPos')).toEqual(s.map); }); }
        if (s.prp) { it(`statement ${ix}/proofPos`, function () { expect(pos(db,ix,'proofPos')).toEqual(s.prp); }); }
    });
}
...
describe('scan empty database:', function () {
    ...
    it('has no statements', function () { expect(db.statements.length).toBe(0); });
    errs([]);
});
...
describe('scan whitespace:', function () {
    ...
    it('is EOF statement', function () { expect(db.statements[0].type).toBe(MMOM.Statement.EOF); });
...
describe('scan ${ token:', function () {
    src('${')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('is OPEN statement', function () { expect(db.statements[0].type).toBe(MMOM.Statement.OPEN); });
    it('containing source', function () { expect(db.statements[0].raw).toBe('${'); });
    errs([]);
});
...
describe('scan ${ token with leading whitespace:', function () {
    ...
    it('is OPEN statement', function () { expect(db.statements[0].type).toBe(MMOM.Statement.OPEN); });
    it('containing source', function () { expect(db.statements[0].raw).toBe('\n\n${'); });
    it('has correct token position', function () { expect(db.statements[0].startPos[1]).toBe(2); });
    errs([]);
});
...
describe('scan ${ token with trailing whitespace:', function () {
    ...
    it('is OPEN statement', function () { expect(db.statements[0].type).toBe(MMOM.Statement.OPEN); });
    it('containing source', function () { expect(db.statements[0].raw).toBe('${'); });
    it('has correct token position', function () { expect(db.statements[0].startPos[1]).toBe(0); });
    it('is EOF statement', function () { expect(db.statements[1].type).toBe(MMOM.Statement.EOF); });
    it('containing source', function () { expect(db.statements[1].raw).toBe('\n\n'); });
    errs([]);
});
...
describe('scan $} token:', function () {
    src('$}')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('is CLOSE statement', function () { expect(db.statements[0].type).toBe(MMOM.Statement.CLOSE); });
    errs([]);
});
...
describe('scan $c statement:', function () {
    ...
    it('has math positions (2)', function () { expect(db.statements[0].mathPos[3]).toBe(6); });
    errs([]);
});
...
describe('scan $c statement with embedded comment:', function () {
    ...
    it('has math positions (length)', function () { expect(db.statements[0].mathPos.length).toBe(4); });
    it('has math positions (1)', function () { expect(db.statements[0].mathPos[1]).toBe(3); });
    it('has math positions (2)', function () { expect(db.statements[0].mathPos[3]).toBe(17); });
    errs([]);
});
...
describe('simple file inclusion:', function () {
    ...
    it('has two statements', function () { expect(db.statements.length).toBe(2); });
    it('first is include per se', function () { expect(db.statements[0].type).toBe(MMOM.Statement.INCLUDE); });
    it('containing source', function () { expect(db.statements[0].raw).toBe('$[ bfile $]'); });
    it('second is close', function () { expect(db.statements[1].type).toBe(MMOM.Statement.CLOSE); });
    it('containing source', function () { expect(db.statements[1].raw).toBe('$}'); });
    errs([]);
});
...
describe('inclusion of root file is ignored:', function () {
    ...
    it('has one statements', function () { expect(db.statements.length).toBe(1); });
    it('first is include per se', function () { expect(db.statements[0].type).toBe(MMOM.Statement.INCLUDE); });
    it('containing source', function () { expect(db.statements[0].raw).toBe('$[ afile $]'); });
    errs([]);
});
...
describe('comment spanning include:', function () {
    ...
    it('has 3 statements', function () { expect(db.statements.length).toBe(3); });
    it('first is include', function () { expect(seg(db,0)).toEqual([MMOM.Statement.INCLUDE,'$[ bfile $]',null,null]); });
    it('second is comment fragment in an EOF', function () { expect(seg(db,1)).toEqual([MMOM.Statement.EOF,'$( comment',null,null]); });
    it('third is trailing portion of wrapping file in an EOF', function () { expect(seg(db,2)).toEqual([MMOM.Statement.EOF,' text $)',null,null]); });
...
describe('nested comment:', function () {
    src('$( foo $( bar $)')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is comment', function () { expect(seg(db,0)).toEqual([MMOM.Statement.COMMENT,'$( foo $( bar $)',null,null]); });
...
describe('not a nested comment:', function () {
    src('$( x$( $a $q $)')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is comment', function () { expect(seg(db,0)).toEqual([MMOM.Statement.COMMENT,'$( x$( $a $q $)',null,null]); });
...
describe('false comment end:', function () {
    src('$( x$)x $)')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is comment', function () { expect(seg(db,0)).toEqual([MMOM.Statement.COMMENT,'$( x$)x $)',null,null]); });
...
describe('unterminated directive:', function () {
    src('$[')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is EOF (uninterpreted)', function () { expect(seg(db,0)).toEqual([MMOM.Statement.EOF,'$[',null,null]); });
...
describe('bad/missing filename:', function () {
    src('$[ $foo $]')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is EOF (uninterpreted)', function () { expect(seg(db,0)).toEqual([MMOM.Statement.EOF,'$[ $foo $]',null,null]); });
    errs([
...
describe('two tokens in include:', function () {
    src('$[ afile bfile $]')
    it('has one statement', function () { expect(db.statements.length).toBe(1); });
    it('first is include (extra token ignored)', function () { expect(seg(db,0)).toEqual([MMOM.Statement.INCLUDE,'$[ afile bfile $]',null,null]); });
...
describe('attempt to span file end with directive:', function () {
    ...
    it('has three statements', function () { expect(db.statements.length).toBe(3); });
    it('first is include', function () { expect(seg(db,0)).toEqual([MMOM.Statement.INCLUDE,'$[ bfile $]',null,null]); });
    it('second is uninterpretable', function () { expect(seg(db,1)).toEqual([MMOM.Statement.EOF,'$[ cfile',null,null]); });
    it('third is uninterpretable', function () { expect(seg(db,2)).toEqual([MMOM.Statement.EOF,' $]',null,null]); });
    errs([
...
describe('missing proof:', function () {
    ...
    it('first is $p', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.PROVABLE,'foo $p x y $.',['x','y'],[],'foo']); });
...
describe('valid $p statement:', function () {
    src('   foo $p x y $= z $( k $) w $.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is $p', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.PROVABLE,'   foo $p x y $= z $( k $) w $.',['x','y'],['z','w'],'foo']); });
...
describe('valid $a statement:', function () {
    src('   foo $a x y $.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is $a', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.AXIOM,'   foo $a x y $.',['x','y'],null,'foo']); });
    it('label position', function () { expect(pos(db,0,'startPos')).toEqual(['afile',3]); });
    it('math positions', function () { expect(pos(db,0,'mathPos')).toEqual(['afile',10,'afile',12]); });
    errs([]);
});
...
describe('spurious proof:', function () {
    src('   foo $f x y $= z $.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is $f', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.BOGUS,'   foo $f x y $= z $.',['x','y','z'],null,'foo']); });
...
describe('spurious proof 2:', function () {
    src('   foo $p x $= y $= z $.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is discarded', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.BOGUS,'   foo $p x $= y $= z $.',['x'],['y','z'],'foo']); });
...
describe('stray $. w/o label:', function () {
    src('$.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is $f', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.BOGUS,'$.',null,null,null]); });
...
describe('stray $. w/ label:', function () {
    src('foo $.')
    it('has 1 statements', function () { expect(db.statements.length).toBe(1); });
    it('first is $f', function () { expect(seg2(db,0)).toEqual([MMOM.Statement.BOGUS,'foo $.',null,null,'foo']); });
