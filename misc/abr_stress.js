    ...
    for (var step_num = 0; step_num < cycles; step_num++) {
        ...
        if (rng() < 0.3) {
            ...
            if (!pool.has(split_res[0])) {
                if (audit && ss.fromArray(ss.toArray(split_res[0])) !== split_res[0]) throw 'Audit failure';
                pool.add(split_res[0]);
                ary.push(split_res[0]);
            }

            if (!pool.has(split_res[1])) {
                if (audit && ss.fromArray(ss.toArray(split_res[1])) !== split_res[1]) throw 'Audit failure';
            ...
            if (ss.length(ary[ix1]) + ss.length(ary[ix2]) > limit) continue;
            var cat_res = ss.concat(ary[ix1], ary[ix2]);
            maxlen = Math.max(maxlen, ss.length(cat_res));

            if (!pool.has(cat_res)) {
                if (audit && ss.fromArray(ss.toArray(cat_res)) !== cat_res) throw 'Audit failure';
                pool.add(cat_res);
                ary.push(cat_res);
            }
        }
