...
export class MMOMStatement {
    ...
    get isAssertion() { return this.type === MMOMStatement.AXIOM || this.type === MMOMStatement.PROVABLE; }
    ...
    get next() {
        ...
        return (db && this.index + 1 < db.statements.length) ? db.statements[this.index + 1] : null;
    }

    get prev() {
        let db = this.database;
        return (db && this.index > 0) ? db.statements[this.index - 1] : null;
    }
...
export class MMOMDatabase {
    ...
    replaceMathString(stmt, text) {
        if (!(stmt instanceof MMOMStatement && stmt.database === this)) throw new TypeError('Not a statement from this database');
        if (!stmt._pos) stmt._unlazy();
        if (stmt._pos.keywordPos < 0 || stmt._pos.endPos < 0) throw new RangeError('Statement does not have a math region');
        let new_stmt_text = stmt.reparse_zone.textSpan(stmt.reparse_index, stmt.reparse_zone.tokenEnd(stmt._pos.keywordPos)) + text +
            stmt.reparse_zone.textSpan(stmt._pos.separatorPos >= 0 ? stmt._pos.separatorPos : stmt._pos.endPos, stmt.reparse_index + stmt.length);
        this._replaceStatements(stmt.index, stmt.index+1, new_stmt_text, 'math');
    }

    replaceProofString(stmt, text) {
        if (!(stmt instanceof MMOMStatement && stmt.database === this)) throw new TypeError('Not a statement from this database');
        if (!stmt._pos) stmt._unlazy();
        if (stmt._pos.separatorPos < 0 || stmt._pos.endPos < 0) throw new RangeError('Statement does not have a proof region');
        let new_stmt_text = stmt.reparse_zone.textSpan(stmt.reparse_index, stmt.reparse_zone.tokenEnd(stmt._pos.separatorPos)) + text +
            stmt.reparse_zone.textSpan(stmt._pos.endPos, stmt.reparse_index + stmt.length);
        this._replaceStatements(stmt.index, stmt.index+1, new_stmt_text, 'proof');
    }
