...
class MMOMMetadata {
    ...
    param(name) {
        ...
        if (type !== 'paramlike' && type !== 'paramarray') throw new RangeError('Not a defined parameter type');
    ...
    _paramLocation(name) {
        let rec = this._parameters.get(name);
        return rec ? MMOMErrorLocation.commentSpan(rec.statement, rec.from, rec.to) : null;
    }
...
['-smm-syntactic-categories','-smm-logical-categories'].forEach(cmd => REGISTRY.set(cmd, 'paramarray')); // NOT STANDARD
...
class MetacommentParser {
    ...
    keyword() {
        ...
        let char;
        while (this.index < this.text.length && /[-0-9._a-zA-Z]/.test(this.text.charAt(this.index)))
            this.index++;
    ...
    paramarray(name) {
        ...
        let old = this.analyzer._parameters.get(name);
        if (old) {
            this.addError('duplicate-parameter', start, this.fieldEnd, { prev: MMOMErrorLocation.commentSpan(old.statement, old.from, old.to) });
        }
        ...
        if (!old) {
            this.analyzer._parameters.set(name, { value: value, statement: this.statement, from: start, to: this.fieldEnd });
        }
        return true;
    }
