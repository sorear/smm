...
export class MMOMStatement {
    ...
    get raw() {
        ...
        for (var i = 0; i < spans.length; i += 3) {
            out += spans[i].text.substring(spans[i+1],spans[i+2]);
        }
        return out;
    }
...
export class MMOMErrorLocation {
    ...
    static _functional(statement, kind, ary, ix) {
        ...
        return new MMOMErrorLocation(kind, statement, ary[ix].source, ary[ix+1], ary[ix].source.tokenEnd(ary[ix+1]), null);
    }
            ...
            if (!this.token_special) {
                switch (state) {
                    case S_IDLE:
                        ...
                        if (posit) this.statement._pos.startPos = [this.zone, this.token_start];
                        state = S_LABEL;
                        break;
                    case S_LABEL:
                    ...
                    case S_MATH:
                        ...
                        if (posit) this.statement._pos.mathPos.push(this.zone, this.token_start);
                        break;
                    case S_PROOF:
                        this.statement.proof.push(token);
                        if (posit) this.statement._pos.proofPos.push(this.zone, this.token_start);
                        break;
                }
                continue;
            }
