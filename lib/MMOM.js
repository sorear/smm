...
export class MMOMStatement {
    ...
    get raw() {
        let from = this.reparse_index + this.reparse_zone.offset;
        return this.reparse_zone.textSpan(from, from + this.length);
    }

    get spans() {
        let from = this.reparse_index + this.reparse_zone.offset;
        return this.reparse_zone.getSpans(from, from + this.length);
    }
    ...
    _unlazy() {
        ...
        scanner.statement._pos = { startPos: -1, mathPos: null, proofPos: null };
...
export class MMOMScanner {
    ...
    newSegment(lt_index) {
        ...
        if (!this.lazyPositions) this.statement._pos = { startPos: -1, mathPos: null, proofPos: null };
