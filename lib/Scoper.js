        ...
        for (segix = 0; segix < statements.length; segix++) {
            ...
            switch (seg.type) {
                ...
                case CONSTANT:
                    ...
                    if (scope_ed_stack.length)
                        this._addError(EL.statement(seg), 'const-not-top-scope', { prev: EL.statement(open_stack[open_stack.length - 1]) });
...
MMOMError.register('scope', 'const-not-top-scope', 'A $c statement is only allowed outside all ${ $} scopes«prev:Open scope brace:l»');
